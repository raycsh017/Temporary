...
extension PrayerViewModel {
	func cellConfigurators() -> [CellConfiguratorType] {
		...
		case .rosary(let mysteryType):
			...
			let prayerFirstPartFormatted = concatenateTexts(of: prependAlternatingBullets(to: finishingPrayer.praise1))
		...
		case .other:
			...
			otherPrayerCellConfigurators = otherPrayers.map {
				let prayerLinesUnderlined = underlineTexts(of: $0.textLines, at: $0.underlineLineOffset)
				let prayerLinesConcatenated = concatenateTexts(of: prayerLinesUnderlined)
				let prayerLinesLineSpacingAdjusted = adjustLineSpacing(of: prayerLinesConcatenated)
				return PrayerOtherPrayerTableCellData(title: $0.title, body: prayerLinesLineSpacingAdjusted)
			}.map {
...
extension PrayerViewModel {
	private func loadRosaryPrayers() {
		...
		rosaryPrayer = rosaryPrayersParsed(fromJSON: rosaryPrayersData)
	}

	private func rosaryPrayersParsed(fromJSON data: Data?) -> [RosaryMysteryKey: RosaryMystery] {
	...
	private func loadOtherPrayers() {
		...
		otherPrayers = otherPrayersParsed(fromJSON: otherPrayersData)
	}

	private func otherPrayersParsed(fromJSON data: Data?) -> [OtherPrayer] {
		guard let data = data else{
			return []
		}
...
// MARK: Formatting
extension PrayerViewModel {
	fileprivate func prependAlternatingBullets(to textLines: [String]) -> [String] {
	...
	fileprivate func underlineTexts(of textLines: [String], at lineOffset: Int?) -> [NSAttributedString] {
	...
	fileprivate func concatenateTexts(of textLines: [String]) -> String {
		return textLines.reduce("") { $0 + $1 }
	}

	fileprivate func concatenateTexts(of attributedTextLines: [NSAttributedString]) -> NSAttributedString {
	...
	fileprivate func adjustLineSpacing(of attributedText: NSAttributedString) -> NSAttributedString {
